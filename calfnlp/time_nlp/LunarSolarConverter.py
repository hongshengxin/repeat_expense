...
def get_holi_date(year, holi):
    month = -1
    day = -1
    if holi == '清明节':
        try:
            date = qingming[str(year)]
            month = date[0]
            day = date[1]
        except:
            month = 4
...
def get_solar_date(year, month, day):
    ...
    for i in range(1, month):
        (leap_month, foo, tmp) = lunar_month_days(year, i)
        if i == leap_month:
            total_days += foo
        total_days += tmp
    (leap_month, foo, tmp) = lunar_month_days(year, month)
    if month == leap_month:
        total_days += tmp
    total_days += day
    tmp = datetime(year, 1, 1) + timedelta(days=total_days-1)
    return {'year': tmp.year, 'month': tmp.month, 'day':tmp.day}
...
# this_month()
# show_month(2018, 1, 15)
# print(get_solar_date(2017, 1, 1))